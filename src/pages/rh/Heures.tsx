...
export default function Heures() {
  ...
  const HR_SHEET_ID = import.meta.env.VITE_HR_SHEET_ID;
  const API_KEY = import.meta.env.VITE_GOOGLE_API_KEY;
;
  const RANGE = 'Hrs!A1:K';
  const objectifHrs = 6500;

  useEffect(() => {
    const fetchSheetData = async () => {
      try {
        const response = await fetch(
          `https://sheets.googleapis.com/v4/spreadsheets/${HR_SHEET_ID}/values/${RANGE}?key=${API_KEY}`
        );
  ...
  >
    ...
    <Box sx={{ 
      ...
      <Typography 
        ...
        sx={{ opacity: 0.8 }}
      >
      {pctObjectif <= 100 ? "Objectif respecté !" : 
       pctObjectif <=113.3 ?"Seuil heures dépassé !":
      ...
      >
        <Typography 
          ...
          fontWeight="700" 
          color={
            pctObjectif <= 100 ? '#00C49F' : 
            //(pctObjectif > 100) && (pctObjectif< 118) ? '#FFBB28' : 
            pctObjectif  < 107.7 ? '#FF8042' :
            '#ff3628'
          }
        >
