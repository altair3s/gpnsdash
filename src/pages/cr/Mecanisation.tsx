...
const parseSheetData = (sheetData: Array<Array<string>>): DayData[] => {
  ...
  return sheetData.slice(1).map(row => {
    ...
    headers.forEach((header, index) => {
      if (header === 'date' || header === 'nom ce' || header === 'infra'|| header === 'lieu'|| header === 'pdf url') {
...
const MEC_SHEET_ID = import.meta.env.VITE_MEC_SHEET_ID;
const API_KEY = import.meta.env.VITE_GOOGLE_API_KEY;
const RANGE = 'Pdf!B1:F';
    ...
    const fetchSheetData = async () => {
      try {
        const response = await fetch(
          `https://sheets.googleapis.com/v4/spreadsheets/${MEC_SHEET_ID}/values/${RANGE}?key=${API_KEY}`
        );
