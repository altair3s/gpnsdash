...
const CR_SHEET_ID = import.meta.env.VITE_CR_SHEET_ID;
const API_KEY = import.meta.env.VITE_GOOGLE_API_KEY;
const RANGE = 'PdfR!D1:G';

const CrVac: React.FC = () => {
    ...
    const fetchSheetData = async () => {
      try {
        const response = await fetch(
          `https://sheets.googleapis.com/v4/spreadsheets/${CR_SHEET_ID}/values/${RANGE}?key=${API_KEY}`
        );
  ...
  return (
    <div className="flex">
      <div className="ml-64 p-2 w-full bg-gray-100">
        ...
        <Box>
        <Paper sx={{ p: 6 }}>
          <Typography gutterBottom variant="h4" sx={{ color: '#004dcf' }}>
          <FaRegFilePdf style={{ fontSize: '2.5rem', color: '#004dcf' }} />
            Compte rendu de vacation
          </Typography>
        </Paper>
        </Box>
        </header>
        ...
        <Grid container spacing={2} style={{ marginTop: '80px' }}>
          <Grid item xs={12}>
            <Paper sx={{ p: 3 }}>
                ...
                <Box sx={{ maxHeight: 500, maxWidth: 500,overflow: 'auto' }}>
                  <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                    <thead style={{ position: 'sticky', top: 0, backgroundColor: '#004dcf', color: 'white' }}>
                      <tr>
