...
const REM_SHEET_ID = import.meta.env.VITE_REM_SHEET_ID;
    ...
    const fetchSheetData = async () => {
      try {
        const response = await fetch(
          `https://sheets.googleapis.com/v4/spreadsheets/${REM_SHEET_ID}/values/${RANGE}?key=${API_KEY}`
        );
  ...
  return (
    ...
    <div className="flex">
      <div className="ml-64 p-2 w-full bg-gray-100">
        ...
        <Box>
        <Paper sx={{ p: 6 }}>
          <Typography gutterBottom variant="h4" sx={{ color: '#006064' }}>
          <FaRegFilePdf style={{ fontSize: '2.5rem', color: '#006064' }} />
            Compte rendu de remise en état
          </Typography>
        </Paper>
        </Box>
        </header>
              ...
              {loading ? (
                ...
                <Box sx={{ maxHeight: 500, maxWidth: 900,overflow: 'auto' }}>
                  <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                    <thead style={{ position: 'sticky', top: 0, backgroundColor: '#006064', color: 'white' }}>
                      <tr>
