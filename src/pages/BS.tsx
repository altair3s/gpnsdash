...
const BS_SHEET_ID = import.meta.env.VITE_BS_SHEET_ID;
const API_KEY = import.meta.env.VITE_GOOGLE_API_KEY;
const RANGE = 'Data!A1:P';
...
const BS: React.FC = () => {
  ...
  const fetchBSData = async (): Promise<void> => {
    try {
      const BS_SHEET_ID = import.meta.env.VITE_BS_SHEET_ID; // Utiliser le même Sheet ID ou le remplacer par celui spécifique aux BS
      const RANGE = 'Data!A:P'; // Ajuster selon la structure de vos données
      
      const bsApiUrl = `https://sheets.googleapis.com/v4/spreadsheets/${BS_SHEET_ID}/values/${RANGE}?key=${API_KEY}`;
  ...
  useEffect(() => {
    ...
    const fetchGoogleSheetsData = async (): Promise<void> => {
      try {
        ...
        const sheetsApiUrl = `https://sheets.googleapis.com/v4/spreadsheets/${BS_SHEET_ID}/values/${RANGE}?key=${API_KEY}`;
